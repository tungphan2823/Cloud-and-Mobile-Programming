...
//                displaySearchResults(results);
                ...
                DatabaseHelper dbHelper = new DatabaseHelper(SearchMeetingActivity.this);

                // Perform the search on the database
                String searchTitle = searchTitleEditText.getText().toString();
                String searchDate = searchDateEditText.getText().toString();
                String searchParticipants = searchParticipantsEditText.getText().toString();
                List<Meeting> results = dbHelper.searchMeetings(searchTitle, searchDate, searchParticipants);
    ...
    private void displaySearchResults(List<Meeting> meetings) {
        ...
        if (meetings.isEmpty()) {
            ...
            for (Meeting meeting : meetings) {
                ...
                // Add image paths
                List<String> imagePaths = meeting.getImagePaths();
                if (!imagePaths.isEmpty()) {
                    results.append("Image Paths: ");
                    for (String imagePath : imagePaths) {
                        results.append(imagePath).append(", ");
                    }
                    results.delete(results.length() - 2, results.length());  // Remove the last comma and space
                    results.append("\n");
                }
